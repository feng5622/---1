...
public class ClientTest 
{
    ...
    private static String ip = "192.168.1.25";// 设置成服务器IP
    private static int port = 54166;
    ...
    private static boolean createConnection() throws Exception//连接服务器
    {
        ...
        {
            JOptionPane.showMessageDialog(new JFrame("温馨提示"), "连接服务器失败，请稍后再试!");
            return false;
        }
    ...
    public static String sendMessage(String fileName) throws SQLException, IOException//发送所看视频文件名称 
    {
    	String path;
    	try 
    	{
    ...
    private static String getMessage() throws IOException //接受视频并将它保存在指定路径下
    {
        if(cs == null)
            return null;
        ...
        {    
            ...
            int count32 = ((int)len/bufferSize + (len%bufferSize > 0 ? 1 : 0));
            float size = 100;
            float add = size/count32 ;
            float Progresslen = 0;
            ...
            {
            	Progresslen += add;
            	//System.out.println(Progresslen);
            	use.printProgress("正在下载...", (int)Progresslen);
  ...
  /*  public static void main(String arg[])
    {
    	...
    	//for(int i = 0; i < 5; i++)
    	{
    		try {
				...
				df.sendMessage("2014-04-12临界资源.avi");
